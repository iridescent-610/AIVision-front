<template>
  ...
  >
    <h1 style="align-self: center">APT软件源管理</h1>
    ...
    <el-dialog title="添加" :visible.sync="dialogInsertVisible" width="1000px">
      <el-form label-position="right" label-width="auto">
        ...
        <br />
        <el-button type="primary" @click="insertAptSource">添加</el-button>
      </el-form>
    </el-dialog>
    <!--    <el-dialog-->
    <!--      title="提示"-->
...
export default {
  ...
  created() {
    ...
    source_api.getAptSourceCount().then((response) => {
      this.totalCount = response.data;
    });

    source_api.getAptSourceListByPage(1, this.pageSize).then((response) => {
  ...
  methods: {
    ...
    refreshPage() {
      ...
      source_api
        .getAptSourceListByPage(this.currentPage, this.pageSize)
        .then((response) => {
          this.list = response.data;
          this.listLoading = false;
        });
