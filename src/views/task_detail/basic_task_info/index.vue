...
<template>
  <div>
    ...
    <b-row>
      <b-col class="mt-1" md="2">教师模型：</b-col>
      <b-col class="mt-1">{{task_details.teacher_models}}</b-col>
    </b-row>

    <b-row v-if="task_details.student_models">
      <b-col class="mt-1" md="2">学生模型：</b-col>
      <b-col class="mt-1">{{task_details.student_models}}</b-col>
    </b-row>
...
export default {
  ...
  computed: {
    ...
    task_state: function () {
      if (this.task_details.processor == "ATLAS") {
        if ("state" in this.task_details && this.task_details.state == "failed")
          return "失败";
